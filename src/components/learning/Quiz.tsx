...
function encodeAsCorrect(q: Question): Answer {
  switch (q.type) {
    ...
    case "fill":
      return { type: "fill", values: q.blanks.map((b) => b.answer) };
    case "label":
      return { type: "label", assignments: q.zones.map((z) => z.label) };
  }
}
...
function encodeAsIncorrect(q: Question): Answer {
  switch (q.type) {
    ...
    case "fill":
      // Empty strings will never match any accepted answer.
      return { type: "fill", values: q.blanks.map(() => "") };
    case "label":
      // No assignments at all — definitely incorrect.
