...
interface Props {
  ...
  type?: CheckType;
  /**
   * Choice list. Each entry is either a plain string (label only) or
   * `{ label, rationale? }`. The rationale shows next to a choice the
   * learner picked and got wrong — prefer this over the generic `hint`
   * so look-alikes can teach the specific trap.
   */
  choices: Choice[];
  /** Index (mcq) or indices (multi) of the correct answer(s). */
  correct: number | number[];
  /** Optional hint shown under the choices after a wrong submit. Generic fallback; per-choice rationale is preferred. */
  hint?: string;
    ...
    >
      ...
      <div className="space-y-1.5" role="group" aria-label={question}>
        {choices.map((choice, i) => {
          ...
          const showRationale =
            !solved && submitted && picked && !isCorrectChoice && rationale;
          return (
            <div key={i}>
              <button
                type="button"
              ...
              {showRationale && (
                <p className="mt-1 ml-1 text-xs leading-relaxed text-destructive/90">
                  {rationale}
                </p>
              )}
            </div>
