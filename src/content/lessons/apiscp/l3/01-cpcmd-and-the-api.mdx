...
Beacon is a small PHP client distributed as a `.phar` (the standard PHP archive format) that calls ApisCP's SOAP API from a remote machine. Useful when scripting from a workstation, a CI runner, or another server. Because it's PHP, the only runtime prerequisite is a PHP 7+ install on the calling host; the repository is at `github.com/apisnetworks/beacon`.
...
The function names use underscore notation (`common_get_load`, not `common:get-load`) but otherwise behave like `cpcmd`. The key Beacon uses is the one a customer (or you, mid-Login-As) issues from **Dev > API Keys** in the panel, introduced in the panel-tour lesson. The key has the same auth scope as the account it was created under.
