...
>
  <Hotspot client:load x={50} y={30} label="1" title="Org logo" purpose="Tells the user the block came from their MSP, not the public internet.">
    Hosted block pages render the org's uploaded logo. End-users should recognise it as their filter, not a generic error page.
  </Hotspot>
  <Hotspot client:load x={50} y={55} label="2" title="Block reason" purpose="Tells the user why the page was blocked, in plain English.">
    Pulls from the matching filter category (e.g. Adult Content) or threat (Phishing). Don't override this with marketing copy.
  </Hotspot>
  <Hotspot client:load x={20} y={60} label="3" title="Domain blocked" purpose="Verifies the call.">
  ...
  <Hotspot client:load x={65} y={75} label="4" title="Notice email link" purpose="User self-serve escalation.">
