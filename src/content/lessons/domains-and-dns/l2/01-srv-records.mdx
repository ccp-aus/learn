...
A client onboarded Teams. You published the two SRVs from the M365 wizard. M365's verification check returns *SRV records not found or incorrect*. `dig SRV _sip._tls.example.com` returns the record but the target field looks like `sipdir.online.lync.com.example.com.` instead of `sipdir.online.lync.com.`.
