...
flowchart LR
  ...
  A["SOC analyst<br/><em>triage, classify,<br/>isolate if high-confidence</em>"]
...
The analyst reads the raw signal, pulls context (the host, the user, recent activity, known false-positive patterns for that customer), decides whether it's a real incident, and writes an Incident Report with a recommendation. When the signal is high-confidence compromise — a canary trip, lateral movement indicators, an identity already inside an active session with malicious actions queued — the SOC will also **isolate the affected entity** (host, identity) before the report reaches you, to prevent the threat spreading while the rest of the response runs. By the time you see the report, three or four things have already happened: triage, classification, recommendation, and (when warranted) containment. The Incident Report is the *output* of analysis, not its input.
...
<DecisionTree client:load
  title="New signal or gut feeling?"
  description="The one legitimate reason to challenge a closed Incident Report is new signal — information the analyst didn't have when they wrote the recommendation. 'My gut says this is wrong' is not new signal. The two get confused under pressure; the tree sorts them."
  startId="root"
  nodes={[
