...
export function remarkGlossaryAutolink() {
  ...
  return (tree, file) => {
    // Skip autolinking outside MDX files. The plugin emits an
    // `mdxJsxTextElement` for the <Term> component; Astro's plain-markdown
    // pipeline (used for .md glossary entries and any other .md source)
    // cannot render that JSX node correctly and falls back to wrapping the
    // children in a <div>, which terminates the surrounding <p> at the HTML
    // parser layer and produces broken inline flow. Only .mdx lessons get
    // glossary autolinking.
    const path = (file?.path || file?.history?.[0] || "").replace(/\\/g, "/");
    // eslint-disable-next-line no-console
    if (typeof process !== "undefined" && process?.env?.DEBUG_GLOSSARY === "1") {
      console.log("[remark-glossary] file.path:", JSON.stringify(file?.path), "history[0]:", JSON.stringify(file?.history?.[0]));
    }
